  ...
  try {
    ...
    if (!isSupportedWithdrawMethod(method)) {
      return res.status(400).json({
        error: 'Unsupported withdrawal method. PayPal is currently the only supported payout method.',
      });
    }
    ...
    // PayPal is the only supported payout method; never debit without a payout attempt.
    ...
    try {
      ...
      if (errorDetails.name === 'AUTHORIZATION_ERROR') {
        return res.status(400).json({
          error: 'PayPal account needs Payouts permission. Check developer.paypal.com',
        });
      }

      return res.status(400).json({
        error: 'PayPal transfer failed. Please check your email/account details.',
      });
    }
