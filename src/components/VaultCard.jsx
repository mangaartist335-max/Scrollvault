...
export default function VaultCard({ balance, floatingEarnings = [] }) {
  ...
  return (
    <motion.div
      ...
      initial="hidden"
      animate="show"
      className={`relative rounded-2xl p-8 text-center border-2 overflow-hidden border-vault-cyan/30 shadow-[0_0_30px_-5px_rgba(56,189,248,0.15)] ${!reduced ? 'animate-border-glow' : ''}`}
    ...
    >
      ...
      {/* Balance display */}
      <motion.div
        className="text-7xl font-black tracking-tight text-vault-green"
        animate={
          isAnimating
