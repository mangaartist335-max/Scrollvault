...
const DEFAULT_FRONTEND =
  normalizeFrontendBase(process.env.FRONTEND_URL) || 'http://localhost:5173';
const ALLOWED_FRONTENDS = new Set(
  [
    DEFAULT_FRONTEND,
    ...(process.env.FRONTEND_URLS || '')
      .split(',')
      .map((url) => normalizeFrontendBase(url))
      .filter(Boolean),
  ]
);

export function resolveFrontendBase(returnTo, allowedFrontends = ALLOWED_FRONTENDS) {
  const candidate = Array.isArray(returnTo) ? returnTo[0] : returnTo;
  const normalized = normalizeFrontendBase(candidate);
  return normalized && allowedFrontends.has(normalized) ? normalized : DEFAULT_FRONTEND;
}
