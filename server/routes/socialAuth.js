...
const DEFAULT_FRONTEND =
  normalizeFrontendOrigin(process.env.FRONTEND_URL) || 'http://localhost:5173';
...
function allowedFrontendOrigins() {
  const origins = new Set([DEFAULT_FRONTEND, ...LOCAL_FRONTEND_ORIGINS]);
  const configured = [
  ...
  for (const value of configured) {
    const origin = normalizeFrontendOrigin(value?.trim());
...
export function resolveFrontendBase(candidate) {
