...
function allowedFrontendOrigins() {
  const configured = [DEFAULT_FRONTEND, ...(process.env.FRONTEND_URLS || '').split(',')];
  return new Set(
    [...configured, ...LOCAL_FRONTEND_ORIGINS]
...
export function resolveFrontendBase(returnTo) {
  const fallback = normalizeOrigin(DEFAULT_FRONTEND) || LOCAL_FRONTEND_ORIGINS[0];
  const requested = normalizeOrigin(returnTo);
  if (!requested) return fallback;
  return allowedFrontendOrigins().has(requested) ? requested : fallback;
}
